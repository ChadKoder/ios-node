angular.module('dash-client').directive('videoInput', ['$q', '$compile', '$timeout', function ($q, $compile, $timeout) {
	return {
		restrict: 'E',
		templateUrl: './views/video-input.html',
		replace: false,
		scope: {
			selectedVideos: '=?'
		},
		link: function (scope, element, attrs, ctrl) {
			...
			var readSelectedFile = function (file, index){
				readAsDataURL(file).then(function(result) {
					...
					if(fileType === 'video' || fileType === 'video/quicktime'){
			...
			scope.$watch('items.length', function() {
				if (scope.items.length === scope.selectedVideos.length){
				}
			});
			...
			for (var i = 0; i < scope.selectedVideos.length; i++){
				if (fileName === scope.selectedVideos[i].fileName){
					imgIndex = i;
				}
			...
			scope.$watch('selectedVideos.length', function() {
				if (scope.selectedVideos.length === scope.totalFiles){
					for (var y=0; y < scope.selectedVideos.length; y++){

						var img = new Image;
						var item = {
							src: scope.selectedVideos[y].dataUrl,
							pid: 'video' + Math.random(),
							w: 0,
							h: 0,
							title: 'my Title!!!!'
						};
