...
}]);/*angular.module('dash-client').controller('PhotosCtrl', ['$scope', 'dialogService', 'httpService', 'appPath',
	function ($scope, dialogService, httpService, appPath) {
		var vm = this;
		...
		$scope.$watch('vm.photos.length', function(i) {
			//if (vm.photos.)
			console.log('got photos in ctrl...');
		  for (var y=0; y < vm.photos.length; y++){  
				var img = new Image;  
				img.src = vm.photos[y].dataUrl;
		...
		vm.submit = function (e){
			...
			if (credsSplit) {
				...
				httpService.post('/files', encodedAuth, formData,{
					transformRequest: angular.identity,
					headers: { 'Content-Type': undefined }
					}).then(function(result) {
						vm.inProgress = false;
